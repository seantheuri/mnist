...
void Model::add(std::string layer, std::vector<int> shape)
{
    ...
    {
        assert(shape.size() == 3 &&
            "Residual block requires 3 parameters: n_kernels, kernel_size, stride");
        layers->push_back(
            new ResidualBlock(last, shape[0], shape[1], shape[2],
                cublasHandle, cudnnHandle));
    }

    else
    {
        std::cerr << "Invalid layer specification: " << layer << std::endl;
