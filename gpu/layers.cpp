...
: Layer(nullptr, cublasHandle, cudnnHandle)
{
    ...
    // Set output tensor descriptor out_shape to have format NCHW, be floats, and have dimensions n, c, h, w
...
void Dense::forward_pass()
{
    ...
    // out_batch = weights^T * in_batch (without biases)
...
void Dense::backward_pass(float learning_rate)
{
    ...
    // grad_in_batch = weights * grad_out_batch
    // Note that grad_out_batch is the next layer's grad_in_batch, and
    // grad_in_batch is the previous layer's grad_out_batch
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    // Get descriptor of input minibatch, in_shape
    CUDNN_CALL( cudnnGetTensor4dDescriptor(in_shape, &dtype, &n, &c, &h, &w,
        &nStride, &cStride, &hStride, &wStride) );

    // Set descriptor of output minibatch, out_shape, to have the same parameters as in_shape and be ordered NCHW
    CUDNN_CALL( cudnnSetTensor4dDescriptor(out_shape, CUDNN_TENSOR_NCHW,
        dtype, n, c, h, w) );
    ...
    // Create activation descriptor, and set it to have the given activationMode, propagate NaN's, and have coefficient coef
    CUDNN_CALL( cudnnCreateActivationDescriptor(&activation_desc) );
    CUDNN_CALL( cudnnSetActivationDescriptor(activation_desc,
        activationMode, CUDNN_PROPAGATE_NAN, coef) );
}

Activation::~Activation()
{
    // TODO (set 5): destroy the activation descriptor
    // Destroy the activation descriptor
...
void Activation::forward_pass()
{
    ...
    // Apply activation, i.e. out_batch = activation(in_batch)
...
void Activation::backward_pass(float learning_rate)
{
    ...
    // Do activation backwards, i.e. compute grad_in_batch
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    // Get the input tensor descriptor details
    CUDNN_CALL( cudnnGetTensor4dDescriptor(in_shape, &dtype, &n, &c, &h, &w,
        &n_stride, &c_stride, &h_stride, &w_stride) );
    ...
    // Create and set the filter descriptor
    CUDNN_CALL( cudnnCreateFilterDescriptor(&filter_desc) );
    CUDNN_CALL( cudnnSetFilter4dDescriptor(filter_desc, dtype, format,
    ...
    CUDNN_CALL( cudnnGetConvolution2dForwardOutputDim(conv_desc,
        in_shape, filter_desc, &out_n, &out_c, &out_h, &out_w) );
    CUDNN_CALL( cudnnCreateTensorDescriptor(&out_shape) );
    CUDNN_CALL( cudnnSetTensor4dDescriptor(out_shape, format, dtype,
...
Conv2D::~Conv2D()
{
    ...
    // Destroy filter_desc and conv_desc
...
void Conv2D::forward_pass()
{
    ...
    // Perform convolution forward pass (store in out_batch).
    // Use class fields workspace and workspace_size for the
    // workspace related arguments in the function call, and
    // use fwd_algo.algo for the algorithm.
    CUDNN_CALL( cudnnConvolutionForward(cudnnHandle,
        ...
        filter_desc, weights,
        conv_desc,
        fwd_algo.algo, workspace, workspace_size,
        &zero,
        out_shape, out_batch) );
...
void Conv2D::backward_pass(float learning_rate)
{
    ...
    CUDNN_CALL( cudnnConvolutionBackwardFilter(cudnnHandle,
        ...
        out_shape, grad_out_batch,
        conv_desc,
        bwd_filter_algo.algo, workspace, workspace_size,
        &zero,
    ...
    // Compute the gradient with respect to the input data in_batch, 
    // and store it in grad_in_batch.
    // Use class fields workspace and workspace_size for the
    // workspace related arguments in the function call and use
    // bwd_data_algo.algo for the algorithm.
    CUDNN_CALL( cudnnConvolutionBackwardData(cudnnHandle,
        ...
        out_shape, grad_out_batch,
        conv_desc,
        bwd_data_algo.algo, workspace, workspace_size,
        &zero,
        in_shape, grad_in_batch) );
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    CUDNN_CALL( cudnnSetPooling2dDescriptor(pooling_desc,
        mode, CUDNN_PROPAGATE_NAN, stride, stride, 0, 0, stride, stride) );
...
Pool2D::~Pool2D()
{
    ...
    // Destroy the pooling descriptor
...
void Pool2D::forward_pass()
{
    ...
    // Do pooling in forward direction, store in out_batch
...
void Pool2D::backward_pass(float learning_rate)
{
    ...
    // Do pooling backwards, store gradient in grad_in_batch
...
void SoftmaxCrossEntropy::forward_pass()
{
    ...
    // Do softmax forward pass using accurate softmax and per instance mode. 
    // Store result in out_batch.
...
void SoftmaxCrossEntropy::backward_pass(float lr)
{
    ...
    //               cublasSaxpy
    // First, copy grad_in_batch = out_batch
    CUDA_CALL( cudaMemcpy(grad_in_batch, out_batch, size * sizeof(float), 
        cudaMemcpyDeviceToDevice) );

    // Set grad_in_batch = grad_in_batch - grad_out_batch using cublasSaxpy
...
ResidualBlock::ResidualBlock(Layer *prev, int n_kernels, int kernel_size, int stride,
    cublasHandle_t cublasHandle, cudnnHandle_t cudnnHandle)
: Layer(prev, cublasHandle, cudnnHandle)
{
    conv1 = new Conv2D(prev, n_kernels, kernel_size, stride, cublasHandle, cudnnHandle);
    act1 = new Activation(conv1, CUDNN_ACTIVATION_RELU, 0.0, cublasHandle, cudnnHandle);
    conv2 = new Conv2D(act1, n_kernels, kernel_size, 1, cublasHandle, cudnnHandle);
    act2 = new Activation(conv2, CUDNN_ACTIVATION_RELU, 0.0, cublasHandle, cudnnHandle);

    // Handle the case where the input and output dimensions are different
    if (prev->get_out_shape() != act2->get_out_shape()) {
        skip_conv = new Conv2D(prev, n_kernels, 1, stride, cublasHandle, cudnnHandle);
    } else {
        skip_conv = nullptr;
    }

    // Set the output shape of the residual block
    cudnnTensorDescriptor_t out_shape_act2;
    cudnnDataType_t dtype;
    int n, c, h, w, nStride, cStride, hStride, wStride;
    out_shape_act2 = act2->get_out_shape();
    CUDNN_CALL(cudnnGetTensor4dDescriptor(out_shape_act2, &dtype, &n, &c, &h, &w,
        &nStride, &cStride, &hStride, &wStride));
...
size_t ResidualBlock::get_workspace_size() const {
    size_t conv1_size = conv1->get_workspace_size();
    size_t conv2_size = conv2->get_workspace_size();
    size_t skip_conv_size = (skip_conv != nullptr) ? skip_conv->get_workspace_size() : 0;
    return std::max({conv1_size, conv2_size, skip_conv_size});
}
...
void ResidualBlock::forward_pass() {
    ...
    if (skip_conv != nullptr) {
        ...
        float alpha = 1.0f;
        float beta = 1.0f;
        CUDNN_CALL(cudnnAddTensor(cudnnHandle, &alpha, skip_conv->get_out_shape(), skip_conv->get_output_fwd(),
            &beta, conv2->get_out_shape(), conv2->get_output_fwd()));
    } else {
        float alpha = 1.0f;
        float beta = 1.0f;
        CUDNN_CALL(cudnnAddTensor(cudnnHandle, &alpha, prev->get_out_shape(), prev->get_output_fwd(),
            &beta, conv2->get_out_shape(), conv2->get_output_fwd()));
    }
...
void ResidualBlock::backward_pass(float learning_rate) {
    act2->backward_pass(learning_rate);

    if (skip_conv != nullptr) {
        float alpha = 1.0f;
        float beta = 0.0f;
        CUDNN_CALL(cudnnAddTensor(cudnnHandle, &alpha, act2->get_out_shape(), act2->get_input_bwd(),
            &beta, skip_conv->get_out_shape(), skip_conv->get_input_bwd()));
        skip_conv->backward_pass(learning_rate);
    } else {
        float alpha = 1.0f;
        float beta = 1.0f;
        CUDNN_CALL(cudnnAddTensor(cudnnHandle, &alpha, act2->get_out_shape(), act2->get_input_bwd(),
            &beta, prev->get_out_shape(), prev->get_input_bwd()));
    }

    conv2->backward_pass(learning_rate);
    act1->backward_pass(learning_rate);
